...
The bookmark-tab status strip reports `<n> saved / MARKS.TXT`. Multiple marks within the same book remain separate rows. The EPUB note is intentionally hidden on the Bookmarks tab while Books and Files keep their existing `TXT / OPEN` presentation and EPUB note.
